...
updateSideFromGID <- function(df) {
    ...
    uni_side <- sapply(uni_GID,function(i){df$Side[!is.na(df$Side) & df$GID == i][1]})
    dat <- data.frame(uni_GID, uni_side)
    for(i in uni_GID){
        df$Side[is.na(df$Side) & df$GID == i] <-  dat$uni_side[dat$uni_GID == i]
    }
    df
}

updateHomePlanetFromSurname <- function(df) {
    earth_sur <- unique(df$LastName[!is.na(df$LastName) & !is.na(df$HomePlanet) & df$HomePlanet == "Earth"])
    mars_sur <- unique(df$LastName[!is.na(df$LastName) & !is.na(df$HomePlanet) & df$HomePlanet == "Mars"])
    europa_sur <- unique(df$LastName[!is.na(df$LastName) & !is.na(df$HomePlanet) & df$HomePlanet == "Europa"])
    uni_surname <- c(earth_sur, mars_sur, europa_sur)
    dat <- data.frame("home" = factor(c(rep("Earth", length(earth_sur)),
                                        rep("Mars", length(mars_sur)),
                                        rep("Europa", length(europa_sur)))),
                      "LastName" = uni_surname)
    for(i in uni_surname){
        df$HomePlanet[is.na(df$HomePlanet) &  !is.na(df$LastName) & df$LastName == i] <- dat$home[dat$LastName == i]
    }
    df
}
...
updateHomePlanetFromDeck <- function(df) {
    ...
    df[!is.na(df$Deck)&df$Deck %in% c("A", "B", "C", "T"), "HomePlanet" ] <- "Europa"
    df
}
