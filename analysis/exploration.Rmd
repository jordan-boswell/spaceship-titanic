...
knitr::opts_knit$set(echo = TRUE, root.dir = '~/Documents/GitHub/spaceship-titanic/data')
```
...
### Update
```{r}
ship[!is.na(ship$Deck)&ship$Deck == "G" & is.na(ship$HomePlanet), "HomePlanet" ] <- "Earth"
ship[!is.na(ship$Deck)&ship$Deck %in% c("A", "B", "C", "T") & is.na(ship$HomePlanet), "HomePlanet" ] <- "Europa"
```
...
### Update
```{r}
uni_side<- sapply(uni_GID,function(i){ship$Side[!is.na(ship$Side) & ship$GID ==i][1]})
dat <- data.frame(uni_GID, uni_side)
for(i in uni_GID){
  ship$Side[is.na(ship$Side) & ship$GID ==i] <-  dat$uni_side[dat$uni_GID ==i]
}
rm(dat, uni_side, uni_GID)
```
...
### Update
```{r}
earth_sur <- unique(ship$LastName[!is.na(ship$LastName) & !is.na(ship$HomePlanet) & ship$HomePlanet == "Earth"])
mars_sur <- unique(ship$LastName[!is.na(ship$LastName) & !is.na(ship$HomePlanet) & ship$HomePlanet == "Mars"])
europa_sur <- unique(ship$LastName[!is.na(ship$LastName) & !is.na(ship$HomePlanet) & ship$HomePlanet == "Europa"])
...
for(i in uni_surname){
  ship$HomePlanet[is.na(ship$HomePlanet)&  !is.na(ship$LastName) & ship$LastName ==i] <- dat$home[dat$LastName==i]
}
rm(dat, uni_surname)
```
...
### Update
```{r}
ship$VIP[!is.na(ship$HomePlanet) & is.na(ship$VIP) & ship$HomePlanet == 'Earth'] <- F
```
