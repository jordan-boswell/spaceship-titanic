...
```{r}
print('Distribution of sleeping people:')
...
print('For each deck, what percentage are sleeping:')
...
ship_cabins <- data.frame(t(
    sapply(splits, function(x){c(
        Size = length(x$PassengerId),
        Cabin = x$Cabin[1],
        Deck = x$Deck[1],
        Side = x$Side[1],
        Num = x$Num[1],
        NumTransported = sum(x$Transported, na.rm=T),
        NumNotTransported = sum(x$Transported==FALSE, na.rm=T),
        NumCryo = sum(x$CryoSleep, na.rm=T)
    )})
))
```
...
## If a person has a group that is all in the same cabin, then
## should we expect that person to also be in that same cabin?
```{r}
