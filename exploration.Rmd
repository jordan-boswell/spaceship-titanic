...
I THINK THIS SECTION NEEDS TO BE RETHOUGHT.  WHAT IF A GROUP HAS MEMBERS LIVING IN LOCATIONS x,x,x,x,y,y,z,q OR x,x,x,x,y?  WHAT ARE WE COUNTING?

THIS IS WRONG.  SEE COMMENT.
```{r}
...
for(i in 1: nrow(ship)){
  ...
  for(j in i:nrow(ship)){
    ...
    if(ship$GID[i] == ship$GID[j] & ship$Cabin[i] != ship$Cabin[j])
        # YOU ARE ADDING THE SAME PERSON'S DECK MULTIPLE TIMES:
...
THIS DENOMINATOR ( length(deck) ) IS NOT USEFUL.
```{r}
