...
```{r}
heat.map <- function(c1, c2){
  train <- subset(ship, Train == T)
  m <- matrix(NA, nrow = length(levels(train[[c1]])), ncol = length(levels(train[[c2]])))
  for (i in 1:nrow(m)){
    for (j in 1:ncol(m)){
      
      m[i,j] <- sum(train$Transported[train[[c1]]==levels(train[[c1]])[i] & train[[c2]]==levels(train[[c2]])[j]]) / nrow(train$Transported[train[[c1]]==levels(train[[c1]])[i] & train[[c2]]==levels(train[[c2]])[j]])
    }
  }
}
```
