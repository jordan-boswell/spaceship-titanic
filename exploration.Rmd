...
for (deck in decks) {
  ship[[paste0("Num", deck)]] <- as.integer(0)
}
...
for (i in 1:nrow(ship)){
  ...
  for (deck in decks) {
      next #DELETE THIS
    #ship[i, paste0("Num", deck, "NA")] <- sum(is.na(group_members$Deck))
    #ship[i, paste0("Num", deck)] <- sum(!is.na(group_members$Deck) & group_members$Deck == deck)
  }
...
for (i in 2:max(ship_groups$Size)){
    print(paste0("All Non NA Same Cabin (For Groups With Size ", i, "):"))
    print(table(ship_groups$AllNonNASameCabin[ship_groups$Size == i]))
    print("-------------------------------------------")
}
